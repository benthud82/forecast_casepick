 ...
 set.seed(500)
 index <- sample(1:nrow(data),round(0.99*nrow(data)))
 train <- data[index,]
 test <- data[-index,]
 ...
 rf <- randomForest(BOXES ~ . , data = train)
 pr.rf <- predict(rf,test)
 MSE.rf = sum((pr.rf - test$BOXES)^2)/nrow(test)
 print(paste(MSE.lm,MSE.nn, MSE.rf))
 plot(test$BOXES,pr.rf,col='blue',main='Real vs predicted Random Forest',pch=18, cex=0.7)
 abline(0,1,lwd=2)
